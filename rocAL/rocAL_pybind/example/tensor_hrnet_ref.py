...
class COCOPipeline(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, seed, data_dir, ann_dir, rali_cpu, display, output_image_width, output_image_height, sigma, is_train, flip_probability, rotate_probability,
                 half_body_probability, scale_factor, rotation_factor):
        ...
        self.coin_flip = ops.CoinFlip(probability=flip_probability)
        self.vertical_flip = ops.CoinFlip(probability=0.0)
        self.horizontal_flip = ops.CoinFlip(probability=flip_probability)
        self.flip = ops.Flip(horizontal_flip_axis=self.horizontal_flip, vertical_flip_axis =  self.vertical_flip)
...
def draw_patches(img, idx):
    ...
    cv2.imwrite(str(idx)+"_"+str(idx2)+"_rali_train.png", image)
...
def main(exp_name,
    ...
    pipe = COCOPipeline(batch_size=bs, num_threads=nt, device_id=di, seed=seed,
                        data_dir=image_path, ann_dir=ann_path, rali_cpu=_rali_cpu, display=display,
                        output_image_width = image_width,output_image_height = image_height,sigma = gauss_sigma,
                        is_train = is_train,flip_probability = flip_prob,rotate_probability=rotate_prob,half_body_probability=half_body_prob,
                        scale_factor=scale_factor,rotation_factor=rotation_factor)
